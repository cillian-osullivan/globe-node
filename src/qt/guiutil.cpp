...
bool parseGlobeURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!GlobeUnits::parse(GlobeUnit::GLB, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatGlobeURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(GlobeUnits::format(GlobeUnit::GLB, info.amount, false, GlobeUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
