...
(define* (make-globe-cross-toolchain target
                                       #:key
...
desirable for building Globe Core release binaries."
...
(define-public glibc-2.27/globe-patched
  (package
    (inherit glibc-2.31)
    (version "2.27")
    (source (origin
    ...
    (cond ((string-suffix? "x86_64-w64-mingw32" target)
          ...
          ((string-contains target "-linux-")
           (list (cond ((string-contains target "riscv64-")
                        (make-globe-cross-toolchain target
                                                      #:base-libc (make-glibc-with-stack-protector
                                                        (make-glibc-with-bind-now (make-glibc-without-werror glibc-2.27/globe-patched)))))
                       (else
                        (make-globe-cross-toolchain target)))))
