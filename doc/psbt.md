# PSBT Howto for Globe Core

Since Globe Core 0.17, an RPC interface exists for Partially Signed Globe
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/globe/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Globe transactions that are not fully signed
...
#### Multisig with multiple Globe Core instances
...
Globe Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `globe-cli` is used.
...
Later, when *V* GLB has been received on *Amulti*, and Bob and Carol want to
