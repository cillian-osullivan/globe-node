#include <globe/globedelegation.h>
...
GlobeDelegation::GlobeDelegation():
    priv(0)
{
    priv = new GlobeDelegationPriv();
}
...
bool GlobeDelegation::GetDelegation(const uint160 &address, Delegation &delegation, Chainstate& chainstate) const
{
...
bool GlobeDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool GlobeDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, ChainstateManager &chainman, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> GlobeDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void GlobeDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool GlobeDelegation::ExistDelegationContract() const
{
    LOCK(cs_main);
...
std::string GlobeDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool GlobeDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{
...
bool GlobeDelegation::IsAddBytecode(const std::vector<unsigned char> &data)
{
...
bool GlobeDelegation::GetUnsignedStaker(const std::vector<unsigned char> &data, std::string &hexStaker)
{
...
bool GlobeDelegation::SetSignedStaker(std::vector<unsigned char> &data, const std::string &base64PoD)
{
    if(!IsAddBytecode(data))
        return false;
