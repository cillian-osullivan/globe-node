This directory contains the source code for the Globe Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Globe UI.
...
- ClientModel is used by the main application `globegui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a globe:... link from a browser).
...
* UI elements like GlobeAmountField, which inherit from QWidget.
* `globestrings.cpp`: automatically generated
* `globeunits.(h/cpp)`: GLB / mGLB / etc. handling
* `callback.h`
...
**Note:** Do not change `local/globe_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `globe-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `globe-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `globe-qt`
9. You're all set! Start developing, building, and debugging the Globe Core GUI
