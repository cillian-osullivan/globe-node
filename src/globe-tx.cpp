...
static void SetupGlobeTxArgs(ArgsManager &argsman)
{
    ...
    argsman.AddArg("-btcmode", "Create globe transactions by default", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    fGlobeMode = !gArgs.GetBoolArg("-btcmode", false);
    fCreateBlank = gArgs.GetBoolArg("-create", false);

    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        // First part of help message is specific to this utility
        std::string strUsage = PACKAGE_NAME " globe-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  globe-tx [options] <hex-tx> [commands]  Update hex-encoded transaction\n"
                "or:     globe-tx [options] -create [commands]   Create hex-encoded transaction\n"
                "\n";
...
{
    int64_t newVersion;
    if (!ParseInt64(cmdVal, &newVersion) || newVersion < 1 || newVersion > TX_MAX_STANDARD_VERSION_GLOBE) {
        throw std::runtime_error("Invalid TX version requested: '" + cmdVal + "'");
    }

    if (!tx.IsGlobeVersion() && IsGlobeTxVersion(newVersion)) {
...
{
    if (!tx.IsGlobeVersion())
        throw std::runtime_error("tx not globe version.");
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        tx.nVersion = CTransaction::CURRENT_GLOBE_VERSION;
        int startArg;
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded globe transaction
