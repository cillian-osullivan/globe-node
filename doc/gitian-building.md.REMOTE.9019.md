...
*Setup instructions for a Gitian build of Globe Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Globe
...
- [Building Globe Core](#building-globe-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-globe-host/README.md).
...
Clone the git repositories for globe and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/globe/globe
git clone https://github.com/globe-core/gitian.sigs.git
```
...
in the globe repository under 'Fetch and create inputs' to install sources which require
...
To build Globe Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the globe repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/globe/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/globe/globe
...
For example:
```bash
URL=https://github.com/laanwj/globe.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit globe=${COMMIT} --url globe=${URL} ../globe/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit globe=${COMMIT} --url globe=${URL} ../globe/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit globe=${COMMIT} --url globe=${URL} ../globe/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the globe git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../globe/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/globe-core/globe-detached-sigs.git

GLBPATH=/some/root/path/globe
SIGPATH=/some/root/path/globe-detached-sigs

./bin/gbuild --url globe=${GLBPATH},signature=${SIGPATH} ../globe/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/globe-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/globe-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/globe-osx-build.assert
```
...
[globe-core/gitian.sigs](https://github.com/globe-core/gitian.sigs/) repository, or if that's not possible create a pull
